    ...
    return async(dispatch) => {
        try {
            ...
            const [orders,
                ...
                Axios({url: `/order/supplier/status?status=cancelled`, method: "GET", withCredentials: true, exposedHeaders: ["set-cookie"]}),
                Axios({url: `/campaigns/All`, method: "GET", withCredentials: true, exposedHeaders: ["set-cookie"]})
            ]);
    ...
    return async(dispatch) => {
        ...
        try {
            const [,
                orders,
                campaigns] = await Promise.all([
                Axios({url: `/order/supplier/cancel`, method: "PUT", data: reject, withCredentials: true}),
                Axios({url: `/order/supplier`, method: "GET", withCredentials: true, exposedHeaders: ["set-cookie"]}),
                Axios({url: `/campaigns/All`, method: "GET", withCredentials: true, exposedHeaders: ["set-cookie"]})
            ]);
            if (orders.data.redirectUrl) { 
              if (orders.data.redirectUrl === '/login') {
